...
export function deliveryAvailableEmptyText(params: Params): string {
  if (!params.customerUuid) return '请先选择货主'
  ...
  if ((excluded?.unassignedWarehouseCount ?? 0) > 0) {
    return `该货主有 ${excluded?.unassignedWarehouseCount} 卷${params.scopeName}尚未分配仓库，暂不能出库`
  }
  if ((excluded?.otherWarehouseCount ?? 0) > 0) {
    return `该货主有 ${excluded?.otherWarehouseCount} 卷${params.scopeName}位于其他仓库`
  }
  if ((excluded?.lockedCount ?? 0) > 0) {
    return `该货主有 ${excluded?.lockedCount} 卷${params.scopeName}已被其他出库单占用`
  }
  if ((excluded?.invalidWeightCount ?? 0) > 0) {
    return `该货主有 ${excluded?.invalidWeightCount} 卷${params.scopeName}缺少有效可用重量`
  }
  return params.hasFilters
    ? `没有符合筛选条件的${params.scopeName}`
    : `该货主在所选仓库暂无可出库${params.scopeName}`
}
