...
export function showBackRecordResult(result: BackRecordResultVO) {
  ...
  const voided = result.orderStatus === 6
  Modal.info({
    title: voided
      ? '回录已关闭，订单已作废'
    ...
    content: (
      <>
        <Descriptions
          className="back-record-result-summary"
          column={2}
          size="small"
        >
          ...
          <Descriptions.Item label="本批母卷">
            {result.recordedRollCount ?? 0} 卷
          </Descriptions.Item>
          <Descriptions.Item label="剩余母卷">
            {result.remainingRollCount ?? 0} 卷
          </Descriptions.Item>
          <Descriptions.Item label="闭合结果">
            <Tag color={CLOSE_LEVEL[check?.level ?? 'PASS']?.color}>
              {CLOSE_LEVEL[check?.level ?? 'PASS']?.text}
            </Tag>
          </Descriptions.Item>
          <Descriptions.Item label="最大偏差率">
            {check?.diffRatioPct == null
              ? '-'
              : formatPercent(check.diffRatioPct)}
          </Descriptions.Item>
          <Descriptions.Item label="直发成品">
            自动生成 {result.directShipGenerated ?? 0} 条
          </Descriptions.Item>
          <Descriptions.Item label="备用号">
            自动作废 {result.voidedSpareCount ?? 0} 个
          </Descriptions.Item>
...
const columns: ColumnsType<RollCheck> = [
  ...
  },
  {
    ...
    width: 100,
    render: (value) => (
      <Tag color={CLOSE_LEVEL[value]?.color}>
        {CLOSE_LEVEL[value]?.text ?? value}
      </Tag>
    ),
  },
  {
    ...
    align: 'right',
    width: 110,
    render: (value) => formatOptionalKg(value),
  },
  {
    title: '理论合计',
