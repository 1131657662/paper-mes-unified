  ...
  return (
    <aside className="back-record-close">
      ...
      <div className="back-record-close__numbers">
        ...
        <Metric label="报废/损耗" value={formatKg(active.scrap)} />
        <Metric label="估算差额" value={active.diff == null ? '-' : `${active.diff.toFixed(3)} kg`} />
      </div>
      <Progress
...
function closureMessage(item: BackRecordWorkItem) {
  ...
  if (item.sourceMode === 'linked') return '当前成品已有来源母卷，后端会按该关系执行逐卷闭合。'
