...
describe('applyQuoteLines', () => {
  it('replaces selected row amounts with the server quote', () => {
    const candidates = [{ orderUuid: 'o1', orderNo: 'JG1', customerUuid: 'c1',
      customerName: '客户', standardProcessAmount: 90, pricingAdjustmentAmount: 10, totalAmount: 100 }]
    const lines = [{ orderUuid: 'o1', sawAmount: 80, rewindAmount: 0, extraAmount: 20,
      standardProcessAmount: 120, pricingAdjustmentAmount: -40,
    ...
    expect(result[0]?.standardProcessAmount).toBe(120)
    expect(result[0]?.pricingAdjustmentAmount).toBe(-40)
  })
})
