...
export default function DocumentAuditTimeline({ logs }: Props) {
  if (logs.length === 0) {
  ...
  return (
    <Timeline
      className="document-audit-timeline"
      items={logs.map((log) => ({
...
function AuditItem({ log }: { log: OperationLog }) {
  return (
    <div className="document-audit-timeline__item">
...
function toneColor(actionType?: string) {
  ...
  if (actionType?.includes('出库') || actionType?.includes('签收')) return 'purple'
  return 'default'
}
