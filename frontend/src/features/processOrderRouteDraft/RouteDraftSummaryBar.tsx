...
export default function RouteDraftSummaryBar({ preview, roll, stages }: Props) {
  ...
  return (
    <div className="route-draft-summary">
      <Statistic title="工序数" value={totals.stageCount} suffix="道" />
      <Statistic title="最终成品" value={totals.finishCount} suffix="件" />
      <Statistic title="预估重量" value={formatKg(totals.finishWeight)} />
      <Statistic title="锯纸刀数" value={totals.knifeCount} suffix="刀" />
      <Statistic title="复卷吨位" value={(totals.rewindWeight / 1000).toFixed(3)} suffix="t" />
      <Statistic
