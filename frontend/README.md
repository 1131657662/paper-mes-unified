...
`openapi:generate` 会运行后端 `OpenApiEnabledContractTest`，抽取 Customer、Paper、Machine、Warehouse 的 GET 契约并更新 `openapi/*-readonly.json` 与 `src/api/generated/*ReadOnly.ts`。不要手工编辑生成文件。CI 使用 `openapi:check` 阻止 Schema 或生成代码未同步的提交。
