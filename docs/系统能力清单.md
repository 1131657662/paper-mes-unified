...
- 无 Flyway/Liquibase 自动迁移（SQL 脚本手动执行 + 启动时 Bootstrap 补表/种子）
- 数据库无外键约束，关联完整性靠应用层
- 上传文件不再公开裸露访问；前端通过 `/api/files/**` 鉴权读取，旧 `/files/**` 路径仅作兼容且同样要求登录
...
- 文件本地存储 `./upload`
- 提供 systemd、Nginx、数据库/上传文件备份、恢复演练脚本样例
- Nginx 样例包含 `/api/auth/login` 站点级限流、CSP Report-Only、`/files/**` 后端鉴权代理
...
- 无健康检查/监控告警接入
- 备份任务不会自动安装；需按部署指南手工部署 cron，并定期做恢复演练
