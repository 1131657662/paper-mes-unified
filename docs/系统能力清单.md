...
- 用户 CRUD、启用/禁用、密码重置（6–32 位）
- 路由级权限守卫 + 侧边栏菜单按权限过滤
- 空库自动创建默认账号：`admin/admin123`、`operator/operator123`

### 不能做什么

- 不能自定义角色或逐权限勾选（仅 4 角色固定映射）
- 不能自助修改个人资料/密码（无个人中心页）
- 不能禁用当前登录用户自身
...
- 快照 diff 对比
- 损伤图片上传（jpg/jpeg/png/webp/gif，单文件 ≤10MB）
...
- 无 Docker 镜像、无 compose、无 Nginx 配置样例
- 无 HTTPS/证书配置
- 无健康检查/监控接入
- 无自动备份
