...
The V3.52 table and constraints are mirrored in the current 3.62 canonical baseline (`sql/01_schema_v4.1.sql`), and the schema-gate contracts require the append-only triggers and balance checks. The V3.50-V3.62 window was replayed successfully in disposable and release environments.

Controlled finished-roll scrap has a dedicated permission-protected command, occupancy checks, reason/request validation, operation log, and SCRAP event. The canonical baseline includes the ledger and `sql/schema-baseline.version` is `3.62`.
