...
public class FinishCustomerRevisionSchemaBootstrap implements ApplicationRunner {
    ...
    private void addCustomerDisplayWeight() {
        Integer count = jdbcTemplate.queryForObject("""
                ...
                WHERE table_schema=DATABASE() AND table_name='biz_finish_roll'
                  AND column_name='customer_display_weight'
                """, Integer.class);
        if (count == null || count == 0) {
            jdbcTemplate.execute("""
                    ALTER TABLE biz_finish_roll ADD COLUMN customer_display_weight DECIMAL(12,3) DEFAULT NULL
                    COMMENT 'Customer-facing weight kg' AFTER customer_finish_width
                    """);
        }
    }
    ...
    private void createRevisionTable() {
        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS biz_finish_customer_revision (
                  uuid VARCHAR(36) NOT NULL, order_uuid VARCHAR(36) NOT NULL,
                  revision_no INT NOT NULL, request_id VARCHAR(64) NOT NULL,
                  request_hash CHAR(64) NOT NULL,
                  source_stage VARCHAR(20) NOT NULL, reason VARCHAR(255) NOT NULL,
                  item_count INT NOT NULL, customer_total_weight DECIMAL(14,3) DEFAULT NULL,
                  ...
                  CONSTRAINT chk_finish_customer_revision_total CHECK
                ...
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Customer-facing finish revisions'
                """);
    }
    ...
    private void createRevisionItemTable() {
        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS biz_finish_customer_revision_item (
                  ...
                  finish_uuid VARCHAR(36) NOT NULL, physical_paper_name VARCHAR(100) NOT NULL,
                  physical_gram_weight INT NOT NULL, physical_finish_width INT NOT NULL,
                  ...
                  customer_finish_width INT NOT NULL, customer_display_weight DECIMAL(12,3) DEFAULT NULL,
                  ...
                  zero_policy VARCHAR(16) NOT NULL DEFAULT 'SKIP', remark VARCHAR(255) DEFAULT NULL,
                  is_deleted TINYINT NOT NULL DEFAULT 0, create_by VARCHAR(50) DEFAULT NULL,
                  update_by VARCHAR(50) DEFAULT NULL,
                  create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
                  update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                  ...
                  CONSTRAINT chk_finish_customer_item_physical CHECK
                    (physical_gram_weight > 0 AND physical_finish_width > 0),
                  CONSTRAINT chk_finish_customer_item_customer CHECK
                    (customer_gram_weight > 0 AND customer_finish_width > 0),
                  CONSTRAINT chk_finish_customer_item_weight CHECK
                    (customer_display_weight IS NULL OR customer_display_weight > 0),
                  CONSTRAINT chk_finish_customer_item_mode CHECK
                    (calculation_mode IN ('KEEP','FIXED','DELTA','RATIO','FORMULA','MANUAL')),
                  CONSTRAINT chk_finish_customer_item_rounding CHECK
                    (rounding_scale BETWEEN 0 AND 3 AND rounding_mode IN ('HALF_UP','UP','DOWN')),
                  CONSTRAINT chk_finish_customer_item_zero CHECK
                    (zero_policy IN ('SKIP','ERROR','USE_ZERO'))
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Customer-facing finish revision items'
                """);
    }
    ...
    private void addWeightOperand() {
        Integer count = jdbcTemplate.queryForObject("""
                SELECT COUNT(*) FROM information_schema.columns
                WHERE table_schema=DATABASE() AND table_name='biz_finish_customer_revision_item'
                  AND column_name='weight_operand'
                """, Integer.class);
        if (count == null || count == 0) {
            jdbcTemplate.execute("""
                    ALTER TABLE biz_finish_customer_revision_item
