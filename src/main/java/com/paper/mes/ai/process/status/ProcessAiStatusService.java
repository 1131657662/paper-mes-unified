...
public class ProcessAiStatusService {
    ...
    public ProcessAiStatusResponse status() {
        ...
        boolean ready = enabled && provider && encryption && memory;
        return new ProcessAiStatusResponse(enabled, ready, "DEEPSEEK",
                ...
                memoryProvider.state(), ready ? null : reason(enabled, provider, encryption, memory));
    }

    private String reason(boolean enabled, boolean provider, boolean encryption, boolean memory) {
